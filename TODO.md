...
- [x] **Menus, Escape, volume and earned speed.** *Implemented 2026-09-10.*
      A main menu (`home.tscn`) after the slot screen — continue, play,
      tech, stats, options, slot, quit; a pause menu in-match with the
      volume sliders and the ways out; Escape as a ladder (close a panel →
      close the pause menu → drop what is selected → pause), and back one
      step on every other screen. Volume left the top bar for sliders.
